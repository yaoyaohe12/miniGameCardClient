...
showBtn.x = windowWidth - showBtn.width
showBtn.y = windowHeight - showBtn.height*3
showBtn.words = "  出牌"

//跳过按钮
var skipBtn = new MyBtn()
skipBtn.x = windowWidth - showBtn.width
skipBtn.y = windowHeight - showBtn.height*5
skipBtn.words = "  跳过"
...
function User(i)
{
  ...
  this.playWords.hide = true
  this.playWords.words = "跳过"
}
...
function initUser()
{
  ...
  userArr[3].btn.y = userArr[0].btn.y+(cardHeight+userPadding)/2 

  userArr[4].btn.x = windowWidth - userImageWidth
  userArr[4].btn.y = userArr[3].btn.y + userImageHeight + userPadding  
}
...
function drawUser()
{
  ...
  for(i=0;i<userMaxCount;i++)
  {
    ...
    if(i<=mylocalId)
    {
      u.playWords.x = u.btn.x + u.btn.width + 5
    }
    else
    {
      u.playWords.x = u.btn.x-40
    }
...
function receiveMsg(res)
{
  ...
  else if(v1[0] == msgTpye.user_info)
  {
    ...
    userArr[getLocalId(receiveId)].btn.words = ab2str(res.message.slice(6))
    if(readyStatus)
    {
  ...
  else if(v1[0] == msgTpye.show_cards_info)
  {
    ...
    if(v3.length == 0)
    {
      userArr[localId].playWords.hide = false
    } 
    else if(receiveId == myId)
    {
      removeCards();
    }
  }
